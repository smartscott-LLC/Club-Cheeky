...
export default function AssetBrowser({ initialCatalog }) {
  ...
  const categorizedCount = Object.values(tiers).filter(
    (t) => t !== 'none'
  ).length;
  ...
  const totalAssets = Object.values(catalog).reduce(
    (s, c) => s + (c.count || 0),
    0
  );
  const totalSizeMB =
    Object.values(catalog).reduce((s, c) => s + (c.totalSizeKB || 0), 0) / 1024;
  ...
  return (
    ...
    >
      ...
      >
        Total: {totalAssets} assets · {totalSizeMB.toFixed(1)} MB · {freeCount}{' '}
        free · {tokenCount} token · {prizeCount} prize · {futureCount} future
      </div>
        ...
        {['all', 'free', 'token', 'prize', 'future'].map((t) => (
          <button
            ...
            style={{
              padding: '8px 20px',
              borderRadius: 6,
              border: `2px solid ${filter === t ? '#FFD800' : '#333'}`,
              background: 'transparent',
              color: filter === t ? '#FFD800' : '#FFB5FF',
              fontFamily: 'Rancho, cursive',
              fontSize: 16,
              cursor: 'pointer'
            }}
      ...
      <div style={{ padding: '0 20px' }}>
        {Object.entries(catalog)
          ...
          .map(([category, data]) => {
            const catFree = (data.assets || []).filter(
              (a) => tiers[a.path] === 'free'
            ).length;
            const catToken = (data.assets || []).filter(
              (a) => tiers[a.path] === 'token'
            ).length;
            const catPrize = (data.assets || []).filter(
              (a) => tiers[a.path] === 'prize'
            ).length;
            const catFuture = (data.assets || []).filter(
              (a) => tiers[a.path] === 'future'
            ).length;
            ...
            return (
              <div key={category} style={{ marginBottom: 20 }}>
                ...
                >
                  ...
                  >
                    {data.count} assets · {catFree} free · {catToken} token ·{' '}
                    {catPrize} prize · {catFuture} future
                  </span>
                </h2>
                <div
                  style={{
                ...
                >
                  {filtered.map((a) => {
                    ...
                    return (
                      <div
                        key={a.path}
                        onClick={() => cycleTier(a.path)}
                        style={{
                          background: '#1a1a2e',
                          border: `2px solid ${tier ? tierColors[tier] : '#333'}`,
                          borderRadius: 8,
                          cursor: 'pointer',
                          transition: 'all 0.2s',
      ...
      >
        <span style={{ color: '#66FFFF' }}>
          {categorizedCount} of {totalAssets} assets categorized
        </span>
        <button
          onClick={() => {
            const entries = Object.entries(tiers)
              .filter(([, t]) => t !== 'none')
              .map(([assetPath, tier]) => ({ path: assetPath, tier }));
