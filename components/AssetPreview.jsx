...
function Viewer({ asset }) {
  const url = asset
    ? `/api/quest/assets/${encodeURIComponent(asset.path)}`
    : null;
...
export default function AssetPreviewPage() {
  ...
  Object.entries(catalog).forEach(([cat, data]) => {
    (data.assets || []).forEach((a) =>
      allAssets.push({ ...a, path: `${cat}/${a.name}` })
    );
  });

  const filtered = allAssets.filter((a) => {
    const matchesCat =
      catFilter === 'all' || a.path.startsWith(catFilter + '/');
    const matchesSearch =
      !search ||
          ...
          {filtered.slice(0, 150).map((a) => (
            <div
              ...
              style={{
                ...
                background:
                  selected?.path === a.path ? '#1a1a2e' : 'transparent'
              }}
            >
              <div
                style={{
              ...
              <div style={{ color: '#66FFFF', fontSize: 11, marginTop: 2 }}>
                {a.sizeKB} KB
              </div>
            </div>
          ))}
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        {selected && (
          ...
          >
            ...
            <div style={{ color: '#66FFFF', fontSize: 12, marginTop: 4 }}>
              {selected.sizeKB} KB · {selected.path.split('/')[0]}
            </div>
          </div>
        )}
      </div>
