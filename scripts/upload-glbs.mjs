...
const SUPABASE_URL =
  ...
  'https://ioqeddpgdilyyajsygmz.supabase.co';
const SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || '';
const SOURCE_DIR =
  process.argv[2] || '/home/server/Pictures/avatar/split_assets';
...
async function main() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (uploaded % 10 === 0 || uploaded === files.length) {
        console.log(
          `Uploaded ${uploaded}/${files.length} (${(totalSize / 1024 / 1024).toFixed(1)}MB)`
        );
      }
    } catch (err) {
      failed++;
  ...
  console.log('');
  console.log(
    `Done: ${uploaded} uploaded, ${failed} failed, ${(totalSize / 1024 / 1024).toFixed(1)}MB total`
  );
